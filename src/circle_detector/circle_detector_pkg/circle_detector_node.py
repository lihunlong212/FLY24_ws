...
class CircleDetectorNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('publish_fine_data', False)
        ...
        self._publish_fine_data = self.get_parameter('publish_fine_data').value
        ...
        self.get_logger().info(
            ...
            f'show_display={self._show} publish_fine_data={self._publish_fine_data}'
        )
    ...
    def _timer_callback(self) -> None:
        ...
        if circle_for_pub is not None:
            ...
            # Match the downstream body-frame convention: target ahead of the UAV is positive Y error.
            pt.point.y = float(cy) - frame_h / 2.0
            pt.point.z = 0.0
        ...
        if self._publish_fine_data and circle_for_pub is not None:
            fine_data_msg = Int32MultiArray()
            fine_data_msg.data = [int(round(pt.point.x)), int(round(pt.point.y))]
            self._pub_fine_data.publish(fine_data_msg)
